...
- **Food** — halal restaurant finder with reviews, favorites, and a food-additives (E-number) checker.
...
- `functions/` — Firebase Cloud Functions: scrapers (`muisScraper`, `googlePlacesScraper`, `muisDiscovery`, `socialMediaChecker`) and `updateManager.ts` that populate Firestore. Separate `package.json`/tsconfig; has its own ESLint.
...
- **Design refresh pass** — the system is strong (Moti, blur, gradients, skeletons). Audit for consistency, add subtle haptics/transitions, and consider a signature "calm" aesthetic (e.g. time-of-day-aware theming tied to prayer times) that becomes the app's visual identity.
